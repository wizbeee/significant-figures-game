...
- 👩‍🏫 **교사 관리 강화** (신규)
  - 🔒 **교실 잠금**: 시험·평가 중 신규 로그인/방 생성 차단
  - ✅ **일괄 승인/거부**: 대기 중인 방 전체를 한 번에 처리
  - 📢 **학생 공지 브로드캐스트**: 전체 또는 특정 방에 토스트 메시지 전달
  - 👁 **인라인 방 상세**: 방 카드를 펼쳐 플레이어별 점수·정답률·진행 상황 실시간 관찰
  - ⏱ **대기 시간 표시**: 승인 대기가 2분 넘으면 경고 하이라이트
  - 📅 **출석 패널**: 오늘 출석/왔다 나감/미접속 한눈에
  - ⭐ **방 프리셋**: 자주 쓰는 설정을 저장하고 드롭다운에서 재사용
  - 📋 **방 복제**: 원클릭으로 같은 설정의 새 방 개설
  - 🔔 **데스크톱 알림 + 소리 + 탭 뱃지**: 승인 대기 발생 즉시 교사에게 알림
  - 🔎 **학생 필터**: 온라인/오프라인/차단/무플레이/오늘 접속 칩 필터
  - 📍 **학생 위치 배지**: 로비/방/문제 진행 번호까지 표시
  - 기본 기능: 방 승인, 점수판 관리, 학생 전적, 구글 시트 동기화
