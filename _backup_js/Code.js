...
function doGet(e) {
  ...
  var file;
  var title = '과학실험실 사용 신청 관리 시스템';
  
  // 라우팅 분기 (✅ 신규 페이지 추가)
  switch(view) {
    case 'admin':
      file = 'admin';
      title = '과학실험실 사용 신청 관리 시스템';
      break;
    case 'report_admin':
      file = 'report_admin';
      title = '보고서 관리';
  ...
  try {
    ...
    return HtmlService.createHtmlOutput(`
      ...
      <style>
        ...
        <div class="container">
          <h1>페이지 설치 필요</h1>
          <p><b>${missing}.html</b> 파일이 없습니다. 다음 순서로 HTML 파일을 추가하세요.</p>
          <div class="step"><b>1단계:</b> GAS 편집기 → <span class="code">파일 → 새로 만들기 → HTML</span></div>
          <div class="step"><b>2단계:</b> 파일명을 <span class="code">${missing}</span> 으로 저장</div>
      ...
      rows.forEach(row=>{
        ...
        hdr.forEach((h,i)=>{
          ...
          if (h==='제출일시' || h==='실험할날짜') {
            ...
            app[h] = d ? d.toISOString() : (v==null ? '' : String(v));
...
function createDataBackup() {
  try {
    ...
    const dst = SpreadsheetApp.create(`실험실신청_백업_${ts}`);
...
function approveWithDuplicateCheck(appId, approvalType) {
  try {
    ...
    logAdminAction('승인', appId, approvalType);
...
function forceApprove(appId, approvalType) {
  try {
    ...
    updateApplicationStatus(appId, updates);
    logAdminAction('강제승인', appId, approvalType + ' (중복 경고 무시)');
...
function listLabGuidanceLogs(){
  try {
    ...
    return rows.map(r=>({
      ...
      '경고여부': String(r[6]||''),
      '지도내용': String(r[7]||''),
      '작성일시': (parseToDateSafe_(r[8]) ? parseToDateSafe_(r[8]).toISOString() : String(r[8]||''))
    }));
  } catch (e) {
    return [];
  }
...
function getRestrictedList(){
  ...
  return rows.map(r=>({
    ...
    '현재상태': String(r[5]||''),
    '최근경고수': Number(r[6]||0),
    '산출일': (parseToDateSafe_(r[7]) ? parseToDateSafe_(r[7]).toISOString() : String(r[7]||''))
  }));
}
...
function saveBackupConfig(retentionDays, enabled) {
  ...
  logAdminAction('설정', '백업설정', `보관기간: ${retentionDays}일, 활성화: ${enabled ? 'Y' : 'N'}`);
...
function setupDailyBackup(hourLocal) {
  ...
  ScriptApp.newTrigger(fn)
    ...
    .everyDays(1)
    .create();
  logAdminAction('설정', '백업 트리거', `매일 ${hour}:00 자동 백업 설정`);
...
function dailyAutoBackup_() {
  try {
    ...
    logAdminAction('백업', '자동백업', `백업 파일 생성: ${backupName}`);
    console.log('자동 백업 완료:', backupName);
  } catch (e) {
    console.error('자동 백업 실패:', e);
    logAdminAction('오류', '자동백업', `백업 실패: ${e.message}`);
  }
}
...
function createManualBackup() {
  try {
    ...
    logAdminAction('백업', '수동백업', `백업 파일 생성: ${backupName}`);
    ...
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][idIdx]) === String(appId)) {
        ...
        if (updateCols.length > 0) {
          ...
          logAdminAction('수정', appId, `임장일정 수정: ${JSON.stringify(updates)}`);
